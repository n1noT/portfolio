...
export default function Projects({
  ...
  useEffect(() => {
    const cursorX = (e: MouseEvent) => {
      ...
      const boxWidth =
        document.querySelector("#projects-list")?.clientWidth ??
        window.innerWidth;
      const closestCursorIndex = Math.round(
        (e.clientX * (projects.length - 1)) / boxWidth
      );
      ...
      projects.forEach((project, index) => {
        ...
        projectElement.style.transform = `perspective(500px) rotateY(${rotationY}deg) ${
          index === closestCursorIndex ? "translateY(-20%)" : ""
        }`;
        ...
        if (index === closestCursorIndex) {
          ...
          projectElement.style.boxShadow =
            "-80px 0 70px -100px var(--danger), 80px 0 70px -100px var(--accent)";
        } else {
    ...
    return () => window.removeEventListener("mousemove", cursorX);
  }, [projects]);
